...
def worker(config,data,error_counter,thread):
    if config['decoder'] == 'spa':
        data = ldpc_python_decoders.spa(data,config['i1'],config['i2'],config['i3'],config['i4'],int(config['num_iterations']))
...
for snr in snrs:
    ...
    while errors < int(config['errors_to_point']):
        ...
        if bool(config['dump_path']):    
            dump_file = open(config['dump_path'],'w')
            dump_file.write(header + f'{snr:.2f}\t\t\t{(errors/bits):.5e}\t\t\t{simspeed/10**6:.5f} Mb/s\n')
            dump_file.close()
        if bool(config['terminal_output']): 
            print(f'{snr:.2f}\t\t\t{(errors/bits):.5e}\t\t\t{simspeed/10**6:.5f} Mb/s',end='\r')
